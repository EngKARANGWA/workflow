...
1. On the Render dashboard, open the service's **Settings** and change **Environment** to `Docker` (or create a new Web Service and select `Docker` up front). Render will then build from the repo's `Dockerfile` instead of auto-detecting Node.
